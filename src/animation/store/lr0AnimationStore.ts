 ...
 * 处理LR0算法的动画数据解析和状态管理
 */
export const useLR0AnimationStore = defineStore('lr0Animation', () => {
  // 状态
  ...
  const parseAnimationData = async (stepData: any) => {
    try {
      ...
      console.log('开始解析LR0动画数据:', stepData)

      // 1. 使用LR栈差异分析器
      const analyzer = AnalyzerFactory.create('LR0', stepData)
      ...
      console.error('解析LR0动画数据失败:', error)
