<template>
  <div class="slr1-record-page min-h-screen theme-main-bg theme-transition">
    <!-- 头部导航 -->
      ...
      <div class="max-w-7xl mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <span class="text-gray-400">|</span>
            <h1 class="text-xl font-semibold text-gray-800">SLR1 分析</h1>
          </div>
          <div class="flex items-center gap-2">
            ...
            <router-link
              to="/slr1"
              class="px-3 py-2 theme-header-text hover:opacity-80 transition-colors font-bold text-cyan-600"
            >
              去答题
            </router-link>
          </div>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8 pt-24">
      <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold theme-content-text">SLR1 历史记录</h1>
            <p class="text-gray-500 text-sm mt-1">系统会自动为您保存最近 50 条验证过的文法及答题进度</p>
          </div>
        </div>
        <div class="flex items-center gap-3">
          <button
            v-if="slr1Store.historyList.length > 0"
      ...
      <!-- 历史记录表格 -->
      <div v-if="slr1Store.historyList.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="w-full text-left border-collapse">
          <thead>
            <tr class="bg-gray-50 border-b border-gray-200">
              ...
              <th class="px-6 py-4 text-xs font-bold text-gray-500 uppercase">输入文法</th>
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="(record, index) in slr1Store.historyList" :key="record.id" class="hover:bg-cyan-50/30 transition-colors group">
              <td class="px-6 py-4 text-sm text-gray-400">{{ index + 1 }}</td>
              <td class="px-6 py-4">
                <span class="px-2 py-1 bg-gray-100 rounded text-xs font-mono text-gray-600 group-hover:bg-cyan-100 group-hover:text-cyan-700 transition-colors">
                  {{ record.id }}
                </span>
              </td>
              <td class="px-6 py-4">
                <div class="max-w-xs truncate font-mono text-sm text-cyan-700 bg-cyan-50 px-2 py-1 rounded" :title="record.grammar">
                  {{ record.grammar.split('\n')[0] }}{{ record.grammar.split('\n').length > 1 ? '...' : '' }}
                </div>
              </td>
              ...
              <td class="px-6 py-4">
                <div class="flex items-center justify-center gap-2">
                  <button
                    ...
                    class="px-4 py-1.5 bg-cyan-600 text-white rounded-lg hover:bg-cyan-700 transition-all text-sm flex items-center gap-1.5 shadow-sm active:scale-95"
                    :disabled="commonStore.loading"
                  >
      ...
      <div v-else class="bg-white rounded-xl shadow-lg p-20 text-center border border-dashed border-gray-300">
        ...
        <p class="text-gray-500 mb-8 max-w-xs mx-auto">您还没有进行过 SLR1 文法分析，或者记录已被清空。</p>
        <router-link
          to="/slr1"
          class="inline-flex items-center px-6 py-2.5 bg-cyan-600 text-white rounded-xl hover:bg-cyan-700 transition-all shadow-md hover:shadow-lg active:scale-95"
        >
          立即前往开始答题
...
import { useSLR1StoreNew, useCommonStore } from '@/stores'
...
const handleClearAll = async () => {
  if (confirm('确定要清空所有记录吗？此操作不可恢复。')) {
    slr1Store.clearAllHistory()
    await slr1Store.persistence.save()
...
const handleDelete = async (recordId: string) => {
  slr1Store.deleteHistoryRecord(recordId)
  await slr1Store.persistence.save()
...
const handleRestore = async (recordId: string) => {
  try {
    ...
    const success = await slr1Store.restoreFromHistory(recordId)
    ...
    console.error('[Record-SLR1] Restore Error:', error)
  } finally {
...
const handleViewReport = (recordId: string) => {
  router.push(`/report/slr1/${recordId}`)
}
