<template>
  <div class="lr0-record-page min-h-screen theme-main-bg theme-transition">
    <!-- 头部导航 -->
      ...
      <div class="max-w-7xl mx-auto px-4 py-4">
        <div class="flex items-center justify-between">
          <div class="flex items-center gap-4">
            ...
            <span class="text-gray-400">|</span>
            <h1 class="text-xl font-semibold text-gray-800">LR0 分析</h1>
          </div>
          <div class="flex items-center gap-2">
            ...
            <router-link
              to="/lr0"
              class="px-3 py-2 theme-header-text hover:opacity-80 transition-colors font-bold text-indigo-600"
            >
              去答题
            </router-link>
          </div>
    ...
    <div class="max-w-7xl mx-auto px-4 py-8 pt-24">
      <div class="flex items-center justify-between mb-8">
        <div class="flex items-center gap-4">
          ...
          <div>
            <h1 class="text-3xl font-bold theme-content-text">LR0 历史记录</h1>
            <p class="text-gray-500 text-sm mt-1">系统会自动为您保存最近 50 条验证过的文法及答题进度</p>
          </div>
        </div>
        <div class="flex items-center gap-3">
          <button
            v-if="lr0Store.historyList.length > 0"
      ...
      <!-- 历史记录表格 -->
      <div v-if="lr0Store.historyList.length > 0" class="bg-white rounded-xl shadow-sm border border-gray-200 overflow-hidden">
        <table class="w-full text-left border-collapse">
          ...
          <tbody class="divide-y divide-gray-100">
            <tr v-for="(record, index) in lr0Store.historyList" :key="record.id" class="hover:bg-indigo-50/30 transition-colors group">
              <td class="px-6 py-4 text-sm text-gray-400">{{ index + 1 }}</td>
              <td class="px-6 py-4">
                <span class="px-2 py-1 bg-gray-100 rounded text-xs font-mono text-gray-600 group-hover:bg-indigo-100 group-hover:text-indigo-700 transition-colors">
                  {{ record.id }}
                </span>
              </td>
              <td class="px-6 py-4">
                <div class="max-w-xs truncate font-mono text-sm text-indigo-700 bg-indigo-50 px-2 py-1 rounded" :title="record.grammar">
              ...
              <td class="px-6 py-4">
                <div class="flex items-center justify-center gap-2">
                  <button
                    ...
                    class="px-4 py-1.5 bg-indigo-600 text-white rounded-lg hover:bg-indigo-700 transition-all text-sm flex items-center gap-1.5 shadow-sm active:scale-95"
                    :disabled="commonStore.loading"
                  >
      ...
      <div v-else class="bg-white rounded-xl shadow-lg p-20 text-center border border-dashed border-gray-300">
        ...
        <p class="text-gray-500 mb-8 max-w-xs mx-auto">您还没有进行过 LR0 文法分析，或者记录已被清空。</p>
        <router-link
          to="/lr0"
          class="inline-flex items-center px-6 py-2.5 bg-indigo-600 text-white rounded-xl hover:bg-indigo-700 transition-all shadow-md hover:shadow-lg active:scale-95"
        >
          立即前往开始答题
...
const handleClearAll = async () => {
  if (confirm('确定要清空所有记录吗？此操作不可恢复。')) {
    lr0Store.clearAllHistory()
    await lr0Store.persistence.save()
...
const handleDelete = async (recordId: string) => {
  lr0Store.deleteHistoryRecord(recordId)
  await lr0Store.persistence.save()
...
const handleRestore = async (recordId: string) => {
  try {
    ...
    const success = await lr0Store.restoreFromHistory(recordId)
    ...
    console.error('[Record-LR0] Restore Error:', error)
  } finally {
...
const handleViewReport = (recordId: string) => {
  router.push(`/report/lr0/${recordId}`)
}
