  ...
  <div class="slr1-step2-report space-y-4">
        ...
        <div class="grid grid-cols-1 lg:grid-cols-2 print:grid-cols-2 gap-4 print:gap-2">
          ...
          <div class="space-y-3">
            <div class="flex items-center gap-2">
              ...
              <h4 class="text-sm font-semibold text-gray-900">学生作答</h4>
              <span
                v-if="isAllCorrect"
            ...
            <div class="space-y-1.5">
              <div
                ...
                :key="`user-${index}`"
                class="flex items-center gap-2 px-3 py-2 rounded-lg border font-mono text-sm font-bold"
                :class="getFormulaClass(formula, index)"
              >
                <span class="w-5 h-5 bg-gray-100 text-gray-600 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0">
                  {{ index + 1 }}
                </span>
                <span class="flex-1 text-base">{{ formula.text || '(未填写)' }}</span>
                <Icon
              ...
              <div v-if="userFormulas.length === 0" class="text-center py-6 text-gray-400">
                <Icon icon="lucide:file-x" class="w-10 h-10 mx-auto mb-2 opacity-50" />
                <p class="text-sm">未填写增广文法</p>
              </div>
            </div>

            <!-- 历史错误记录 -->
            <div v-if="historyErrors.length > 0" class="mt-3">
              <div class="flex items-start gap-2 p-2 bg-red-50 border border-red-200 rounded-lg">
                <Icon icon="lucide:history" class="w-3.5 h-3.5 text-red-600 mt-0.5 flex-shrink-0" />
                <div class="text-xs text-red-800 flex-1">
                  <p class="font-medium mb-1">历史错误：</p>
                  <div class="flex flex-wrap gap-1.5">
                    <span
                      v-for="(err, idx) in historyErrors"
                      :key="`hist-${idx}`"
                      class="px-1.5 py-0.5 bg-red-100 text-red-600 rounded text-xs font-mono line-through border border-red-200"
                    >
                      {{ err.wrongValue }}
                    </span>
                  </div>
            ...
            <div class="flex flex-wrap gap-3 text-xs pt-1">
              <div class="flex items-center gap-1">
                ...
                <span class="text-gray-600">回答正确</span>
              </div>
              <div class="flex items-center gap-1">
                <span class="w-2.5 h-2.5 bg-red-400 rounded-full"></span>
                <span class="text-gray-600">历史错误</span>
              </div>
            </div>
          </div>
          ...
          <div class="space-y-3">
            ...
            <div class="space-y-1.5">
              <div
                ...
                :key="`correct-${index}`"
                class="flex items-center gap-2 px-3 py-2 rounded-lg border border-green-200 bg-green-50 font-mono text-sm"
              >
                <span class="w-5 h-5 bg-green-200 text-green-700 rounded-full flex items-center justify-center text-xs font-semibold flex-shrink-0">
                  {{ index + 1 }}
                </span>
                <span class="flex-1 text-green-800 text-base font-semibold">{{ formula }}</span>
                <Icon icon="lucide:check" class="w-4 h-4 text-green-600 flex-shrink-0" />
              </div>
              <div v-if="correctFormulas.length === 0" class="text-center py-6 text-gray-400">
                <Icon icon="lucide:loader-2" class="w-10 h-10 mx-auto mb-2 opacity-50 animate-spin" />
            ...
            <div class="p-2.5 bg-blue-50 border border-blue-200 rounded-lg">
              <div class="flex items-start gap-2">
                <Icon icon="lucide:lightbulb" class="w-3.5 h-3.5 text-blue-500 mt-0.5 flex-shrink-0" />
                <div class="text-xs text-blue-700">
                  <p class="font-medium mb-1">增广文法构造规则：</p>
                  <ul class="space-y-0.5 text-xs leading-relaxed">
                    <li>• 添加新的开始符号 {{ startSymbol }}'</li>
                    <li>• 添加产生式：{{ startSymbol }}' -> {{ startSymbol }}</li>
                    <li>• 分解多候选式：A -> α | β 分为 A -> α 和 A -> β</li>
                  </ul>
                </div>
              </div>
            </div>
    ...
    <div class="grid grid-cols-3 gap-3">
      <div class="bg-white rounded-lg border border-gray-200 p-3">
        <div class="text-xl font-bold text-gray-900">{{ userFormulas.length }}</div>
        <div class="text-xs text-gray-500">填写产生式数</div>
      </div>
      <div class="bg-white rounded-lg border border-gray-200 p-3">
        <div class="text-xl font-bold text-green-600">{{ correctCount }}</div>
        <div class="text-xs text-gray-500">正确数量</div>
      </div>
      <div class="bg-white rounded-lg border border-gray-200 p-3">
        <div class="text-xl font-bold" :class="accuracyClass">{{ accuracy }}%</div>
