      ...
      <div v-else class="max-w-7xl mx-auto">
        ...
        <div class="bg-gradient-to-br from-slate-50 via-blue-50 to-indigo-50 rounded-xl p-6 mb-6 border border-slate-200 shadow-sm">
          ...
          <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <!-- First集 -->
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-5 border border-blue-200/50 shadow-sm">
              <div class="flex items-center mb-4">
                <div class="w-8 h-8 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg flex items-center justify-center mr-3">
                  <Icon icon="lucide:arrow-right" class="w-4 h-4 text-white" />
                </div>
                <h4 class="text-lg font-semibold text-emerald-800">First集（非终结符）</h4>
              </div>
              <div class="space-y-3">
                <div v-for="(symbols, symbol) in firstSets" :key="`first-${symbol}`" class="group">
                  <!-- 只显示非终结符的First集 -->
                  <template v-if="originalData?.Vn.includes(symbol)">
                    <div class="flex items-center p-3 rounded-lg border border-emerald-200/50 bg-gradient-to-r from-emerald-50/50 to-teal-50/50 hover:from-emerald-100/70 hover:to-teal-100/70 transition-all duration-200 shadow-sm hover:shadow-md"
                         :class="{ 'ring-2 ring-yellow-400 ring-opacity-60 shadow-lg': symbolHighlightState[symbol] }">
                      <div class="w-12 h-12 bg-gradient-to-br from-emerald-500 to-teal-600 rounded-lg flex items-center justify-center mr-3 shadow-sm">
                        <span class="font-mono font-bold text-white text-lg">{{ symbol }}</span>
                      </div>
                      <div class="flex-1">
                        <div class="text-sm text-emerald-700 font-medium mb-1">First({{ symbol }})</div>
                        <div class="flex flex-wrap gap-1">
                          <span
                            v-for="(sym, index) in symbols"
                            :key="`${symbol}-${sym}-${index}`"
                            class="inline-flex items-center px-2 py-1 text-xs font-mono bg-emerald-100 text-emerald-800 rounded-md border border-emerald-200 shadow-sm"
            ...
            <!-- Follow集 -->
            <div class="bg-white/70 backdrop-blur-sm rounded-xl p-5 border border-indigo-200/50 shadow-sm">
              <div class="flex items-center mb-4">
                <div class="w-8 h-8 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center mr-3">
                  <Icon icon="lucide:arrow-left" class="w-4 h-4 text-white" />
                </div>
                <h4 class="text-lg font-semibold text-indigo-800">Follow集</h4>
              </div>
              <div class="space-y-3">
                <div v-for="(symbols, symbol) in followSets" :key="`follow-${symbol}`" class="group">
                  <div class="flex items-center p-3 rounded-lg border border-indigo-200/50 bg-gradient-to-r from-indigo-50/50 to-purple-50/50 hover:from-indigo-100/70 hover:to-purple-100/70 transition-all duration-200 shadow-sm hover:shadow-md"
                       :class="{ 'ring-2 ring-yellow-400 ring-opacity-60 shadow-lg': symbolHighlightState[symbol] }">
                    <div class="w-12 h-12 bg-gradient-to-br from-indigo-500 to-purple-600 rounded-lg flex items-center justify-center mr-3 shadow-sm">
                      <span class="font-mono font-bold text-white text-lg">{{ symbol }}</span>
                    </div>
                    <div class="flex-1">
                      <div class="text-sm text-indigo-700 font-medium mb-1">Follow({{ symbol }})</div>
                      <div class="flex flex-wrap gap-1">
                        <span
                          v-for="(sym, index) in symbols"
                          :key="`${symbol}-${sym}-${index}`"
                          class="inline-flex items-center px-2 py-1 text-xs font-mono bg-indigo-100 text-indigo-800 rounded-md border border-indigo-200 shadow-sm"
                          :class="{ 'bg-yellow-200 border-yellow-300 ring-1 ring-yellow-400': symbolHighlightState[symbol] }"
                        >
                          {{ sym === '#' ? '#' : sym }}
                        </span>
