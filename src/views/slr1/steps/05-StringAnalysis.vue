    ...
    <div class="step-content">
      ...
      <div class="grid grid-cols-1 lg:grid-cols-2 gap-6 mb-6">
        ...
        <div class="bg-purple-50 border border-purple-200 rounded-lg p-6">
          <div class="flex items-start">
            <Icon icon="lucide:file-text" class="w-5 h-5 text-purple-600 mt-0.5 mr-3" />
            <div class="flex-1">
              <h3 class="text-lg font-semibold text-purple-900 mb-2">当前文法</h3>
              <div v-if="grammarInfo" class="space-y-2">
                <div class="bg-white/60 rounded-lg p-3 border border-purple-200">
                  <div class="text-sm text-purple-800 font-medium mb-2">产生式：</div>
                  <div class="space-y-1">
                    <div
                  ...
                  <tr v-for="(step, index) in analysisSteps" :key="index" class="hover:bg-gray-50">
                    ...
                    <td class="px-2 py-1 border border-gray-300 text-xs">
                      <input
                        ...
                        class="w-full px-1 py-0.5 text-xs text-center border-0 focus:ring-1 focus:ring-purple-500 rounded transition-colors"
                        placeholder="如: 0"
                      />
                      <span
                        v-else
    ...
    for (let i = 0; i < result.info_step.length; i++) {
      ...
      if (Array.isArray(stateStack)) {
        ...
        if (stateStack && !stateStack.includes(' ')) {
          ...
          if (slr1Store.actionTable) {
            Object.keys(slr1Store.actionTable).forEach((key) => {
...
const isStepComplete = computed(() => slr1Store.inputAnalysisResult !== null)
...
const stateCount = computed(() => {
  if (!slr1Store.actionTable) return 0
...
const getActionValue = (state: number, terminal: string) => {
  ...
  let value = slr1Store.actionTable[key]
...
const getGotoValue = (state: number, nonterminal: string) => {
  ...
  let value = slr1Store.gotoTable[key]
...
根据SLR1分析规则，使用状态栈顶状态和输入串首符号查询Action表。`,
    '观察高亮区域',
    3000,
    'bottom-left'
  )
...
async function executeStatePopAnimation(popCount: number, rowIndex: number) {
  ...
  // 获取当前状态栈 - 修复：正确处理空格分隔的状态
  const currentStateStack = userAnswers.value.stateStack[rowIndex]
  // 确保状态栈是字符串格式，然后按空格分割
  const stateStackStr = String(currentStateStack)
  const states = stateStackStr.split(/\s+/).filter((s) => s.trim() !== '')
